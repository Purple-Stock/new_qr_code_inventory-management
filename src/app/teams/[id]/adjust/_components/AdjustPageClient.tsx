...
export function AdjustPageClient({ items, locations, team }: AdjustPageClientProps) {
  ...
  const handleStockChange = (itemId: number, newStock: number) => {
    ...
    setSelectedItems(
      selectedItems.map((si) => (si.item.id === itemId ? { ...si, newStock } : si))
    );
  };
    ...
    try {
      ...
      if (firstError) {
        ...
        toast({
          variant: "destructive",
          title: t.common.error,
          description: firstError.error || t.adjust.partialAdjustError,
        });
        return;
      }
    ...
    <TeamLayout team={team} activeMenuItem="adjust">
      ...
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm sm:text-base md:text-lg text-gray-600">{t.adjust.subtitle}</p>
        </div>
        <Button
          variant="outline"
                  ...
                  <tr key={selectedItem.item.id} className="hover:bg-purple-50/50 transition-colors">
                    ...
                    <td className="px-4 sm:px-6 py-4 sm:py-5">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() =>
                            handleStockChange(selectedItem.item.id, selectedItem.newStock - 1)
                          }
