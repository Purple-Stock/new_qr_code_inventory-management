...
export function parseStockTransactionPayload(body: unknown): ValidationResult<{
  ...
  if (typeof transactionType !== "string" || !allowedTypes.includes(transactionType as StockTransactionType)) {
    return { ok: false, error: "Missing required fields" };
  }
...
export function parseStockActionInput(
  body: unknown,
  mode: "stock_in" | "stock_out" | "adjust" | "move"
): ValidationResult<{
  itemId: number;
  quantity: number;
  notes: string | null;
  ...
  const itemIdParsed = parseNumber(body.itemId, "itemId");
  const quantityParsed = parseNumber(body.quantity, "quantity");
  if (!itemIdParsed.ok || !quantityParsed.ok || !Number.isInteger(itemIdParsed.data)) {
    return { ok: false, error: "Missing required fields" };
  }
  if (quantityParsed.data <= 0) {
    return { ok: false, error: "Quantity must be greater than 0" };
  ...
  if (mode === "move") {
    const sourceParsed = parseOptionalInteger(body.sourceLocationId);
    const destinationParsed = parseOptionalInteger(body.destinationLocationId);
    if (!sourceParsed.ok || !destinationParsed.ok) {
      return { ok: false, error: "Location IDs must be integers" };
    }
    return {
      ok: true,
      data: {
        ...
        quantity: quantityParsed.data,
        notes: notesParsed.data ?? null,
        sourceLocationId: sourceParsed.data ?? null,
        destinationLocationId: destinationParsed.data ?? null,
      },
    };
  }

  const locationParsed = parseOptionalInteger(body.locationId);
  ...
  return {
    ...
    data: {
      ...
      quantity: quantityParsed.data,
      notes: notesParsed.data ?? null,
      sourceLocationId: mode === "stock_out" ? locationParsed.data ?? null : null,
      destinationLocationId:
        mode === "stock_in" || mode === "adjust" ? locationParsed.data ?? null : null,
    },
  };
}
